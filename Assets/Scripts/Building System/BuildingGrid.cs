...
public class BuildingGrid : MonoBehaviour
{
    ...
    {
        // Map all intended world positions to grid coordinates (unique)
        var intended = new System.Collections.Generic.HashSet<(int x, int y)>();
        foreach (var position in allBuildingPositions)
        {
            (int x, int y) = WorldToGridPosition(position);
            intended.Add((x, y));
        ...
        // Validate each intended cell: inside bounds and not occupied by other buildings
